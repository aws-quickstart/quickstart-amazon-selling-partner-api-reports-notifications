...
. In the imported Postman collection, choose the *STS Assume API Gateway Role* request. 
. Choose *Send* to submit the request. The request retrieves the temporary credentials from the *ApiGatewayInvokeRole* role and automatically stores them in collection variables.
. In the imported Postman collection, choose the *Token Storage* request.
...
. In the imported Postman collection, choose the *Notifications Subscriber* request.
...
. In the imported Postman collection, choose the *Report Creator* request.
